...
// ตรวจสอบว่ามีการส่งแบบ POST หรือไม่
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($card_number) || empty($expiration_date) || empty($cvv) || empty($card_holder)) {
        echo "<script>alert('กรุณากรอกข้อมูลให้ครบถ้วน'); window.location.href='checkout.php';</script>";
        exit();
    }
    ...
    // บันทึกคำสั่งซื้อ
    $query = "INSERT INTO orders (total_amount, card_holder, card_number) VALUES ('$total', '$card_holder', '$card_number')";
    if (mysqli_query($conn, $query)) {
        // ล้างตะกร้าสินค้า
        unset($_SESSION['cart']);

        // เปลี่ยนเส้นทางไปหน้า index.php พร้อมแจ้งเตือน
        echo "<script>alert('ชำระเงินสำเร็จ!'); window.location.href='user_index.php';</script>";
    } else {
        echo "<script>alert('เกิดข้อผิดพลาด! กรุณาลองใหม่'); window.location.href='checkout.php';</script>";
    }
